...
$type = $_GET['type'] ?? ($_POST['doc_type'] ?? '');
if (!isset($docTypes[$type])) {
    flash('danger', 'Invalid document type.');
    redirect('documents/index.php');
}

$config = $docTypes[$type];
$pageTitle = 'Create ' . $config['label'];
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Gather type-specific data fields into JSON
    $data = [];
    ...
    if (!$errors) {
        $db->beginTransaction();
        try {
            $stmt = $db->prepare('INSERT INTO documents (user_id, doc_type, title, project_name, data, status) VALUES (?, ?, ?, ?, ?, ?)');
            $stmt->execute([
                $user['id'], $type, $title, $project_name ?: null,
            ...
            foreach ($items as $item) {
                $itemStmt->execute([
                    $docId, $item['item_no'], $item['description'],
            ...
            $db->commit();
            flash('success', $config['label'] . ' created successfully.');
            redirect('documents/view.php?id=' . $docId);
        } catch (Exception $e) {
            $db->rollBack();
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <h5 class="fw-bold mt-1 mb-0">Create <?= h($config['label']) ?></h5>
    </div>
</div>
...
<form method="POST">
        ...
        <div class="card-body">
            <div class="row g-3">
                ...
                <div class="col-md-<?= $field['type'] === 'textarea' ? '12' : '6' ?>">
                    ...
                    <textarea class="form-control" name="data_<?= $key ?>" rows="3"><?= h($_POST["data_{$key}"] ?? '') ?></textarea>
                    <?php elseif ($field['type'] === 'date'): ?>
                    <input type="date" class="form-control" name="data_<?= $key ?>" value="<?= h($_POST["data_{$key}"] ?? '') ?>">
                    <?php elseif ($field['type'] === 'number'): ?>
                    <input type="number" class="form-control" name="data_<?= $key ?>" step="0.01" value="<?= h($_POST["data_{$key}"] ?? '') ?>">
                    <?php else: ?>
                    <input type="text" class="form-control" name="data_<?= $key ?>" value="<?= h($_POST["data_{$key}"] ?? '') ?>">
                    <?php endif; ?>
                </div>
                <?php endforeach; ?>
                <div class="col-md-6">
    ...
    <div class="d-flex gap-2">
        ...
        <a href="<?= url('documents/index.php') ?>" class="btn btn-outline-secondary">Cancel</a>
    </div>
</form>
